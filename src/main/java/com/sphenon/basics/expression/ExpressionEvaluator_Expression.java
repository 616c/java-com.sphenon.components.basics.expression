...
public class ExpressionEvaluator_Expression implements ExpressionEvaluator {

    public ExpressionEvaluator_Expression (CallContext context) {
    ...
    public String[] getIds(CallContext context) {
        return new String[] { "e", "expression" };
    }

    public Object evaluate(CallContext context, String string, Scope scope) throws EvaluationFailure {
        com.sphenon.basics.expression.parsed.Expression expression;
        try {
            expression = com.sphenon.basics.expression.parsed.ExpressionParser.parse(context, string);
        } catch (com.sphenon.basics.expression.parsed.ParseException pe) {
            EvaluationFailure.createAndThrow(context, pe, "Syntax error in expression '%(string)'", "string", string);
            throw (EvaluationFailure) null;
        }

        return expression.getValue(context, scope);
    }
