...
public class Test_ActivityHandlerExpressions extends com.sphenon.basics.testing.classes.TestBase {
    ...
    public Test_ActivityHandlerExpressions (CallContext context) {
    }

    public String getId(CallContext context) {
        if (this.id == null) {
            this.id = "ActivityHandlerExpressions";
        }
        return this.id;
    }
    ...
    public TestResult perform (CallContext call_context, TestRun test_run) {
        ...
        try {
            ...
            for (TestCase test_case : getTestCases(context)) {
                ...
                ActivityHandler ah = ActivityHandler_Expression.create(context, test_case.getE(context));
                if (ah.execute(context, null).isValid(context)) {
                    // ah.dumpData(context);

                    Object result = ah.getSlotValue(context, 0);
                    ...
                    return new TestResult_Execution(context, ah.getExecution(context));
                }
