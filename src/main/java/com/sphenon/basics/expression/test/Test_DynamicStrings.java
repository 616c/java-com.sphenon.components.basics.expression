...
public class Test_DynamicStrings extends com.sphenon.basics.testing.classes.TestBase {
    ...
    static public class TestCase {
        ...
        protected String ds;
        public String getDS(CallContext context) {
            return this.ds;
        }
        public void setDS(CallContext context, String ds) {
            this.ds = ds;
        }
    ...
    public TestResult perform (CallContext call_context, TestRun test_run) {
        ...
        try {
            ...
            for (TestCase test_case : getTestCases(context)) {
                ...
                String result = (new DynamicString(context, test_case.getDS(context))).get(context, test_case.getArguments(context).getArguments(context));

                if ((notification_level & Notifier.CHECKPOINT) != 0) { NotificationContext.sendCheckpoint(context, "DynamicString: '%(dynamic)' ==> '%(result)'", "dynamic", test_case.getDS(context), "result", result); }
            }
