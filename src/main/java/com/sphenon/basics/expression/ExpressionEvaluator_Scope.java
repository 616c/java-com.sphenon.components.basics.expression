    ...
    public Object evaluate(CallContext context, String instruction, Scope scope) throws EvaluationFailure {
        ...
        if ((matches = get_instruction.tryGetMatches(context, instruction)) != null) {
            ...
            if (scope == null) {
                ...
                EvaluationFailure.createAndThrow(context, "No scope provided while trying to get variable '%(name)'", "name", varname);
                throw (EvaluationFailure) null;
            }

            try {
                return optional ? scope.tryGet(context, varname) : scope.get(context, varname);
            } catch (NoSuchVariable nsv) {
                EvaluationFailure.createAndThrow(context, "No such variable '%(name)' in scope", "name", varname);
                throw (EvaluationFailure) null;
            }
            ...
            if (scope == null) {
                EvaluationFailure.createAndThrow(context, "No scope provided while trying to set variable '%(name)'", "name", targetname);
                throw (EvaluationFailure) null;
            }
            ...
            if (matches[1] != null) {
                ...
                if (sourcename.equals("true")) {
                    ...
                    try {
                        ...
                        EvaluationFailure.createAndThrow(context, "No such variable '%(name)' in scope", "name", sourcename);
                        throw (EvaluationFailure) null;
                    }
                }
            ...
            if (scope == null) {
                EvaluationFailure.createAndThrow(context, "No scope provided while trying to add parent");
                throw (EvaluationFailure) null;
            }
            ...
            if ((scope instanceof Class_Scope) == false) {
                EvaluationFailure.createAndThrow(context, "Scope is not modifyable while trying to add parent");
                throw (EvaluationFailure) null;
            }

            if ((value instanceof Scope) == false) {
                EvaluationFailure.createAndThrow(context, "Value is not a Scope while trying to add parent");
                throw (EvaluationFailure) null;
            }
            ...
            EvaluationFailure.createAndThrow(context, "Unknown instruction '%(instruction)' in scope expression", "instruction", instruction);
            throw (EvaluationFailure) null;
        }
    }
