...
public class Equal extends ExpressionBase {
    ...
    public Equal (CallContext context, Expression c1, Expression c2) {
        this(context, c1, c2, -1, -1);
    }

    public Equal (CallContext context, Expression c1, Expression c2, int lower_limit, int upper_limit) {
        super(context);
        this.c1 = c1;
        this.c2 = c2;
    ...
    public Object getValue(CallContext context, Scope scope) throws EvaluationFailure {
        if (c1 instanceof Array) {
            ...
            return (    (lower_limit == -1 ? equal >= size :
                         lower_limit == -2 ? equal >= size && size >= 1 :
                                             equal >= lower_limit)
                     && (upper_limit == -1 || upper_limit == -2 ? equal <= size : equal <= upper_limit)
                   );
        } else {
            return compare(context, c1, c2, scope);
        }
    }
