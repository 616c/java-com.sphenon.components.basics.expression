...
public class ExpressionEvaluator_SSHNew extends ExpressionEvaluator_BashNew {

    protected String ip;
    protected String port;

    public ExpressionEvaluator_SSHNew (CallContext context, String ip, String port) {
        super(context,
              "SSH${actor_id;_}@" + ip + (port == null ? "" : ("(" + port + ")")),
              "ssh" + (port == null ? "" : (" -p " + port)) + " -A -o ServerAliveInterval=120 -t -t ${actor_id;;@}" + ip + " /bin/bash --noprofile --norc -i",
              true);
    ...
    public ExpressionEvaluator_SSHNew (CallContext context, String ip) {
